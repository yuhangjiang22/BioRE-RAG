...
class BasePLModule(pl.LightningModule):
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> None:
        ...
        with torch.no_grad():
            ...
            forward_output = {}
            forward_output['loss'], forward_output['logits'] = self.forward(batch, labels)
        ...
        # outputs = {}
        predictions, labels = self.generate_triples(batch, labels)
    ...
    def test_step(self, batch: dict, batch_idx: int) -> None:
        ...
        if self.hparams.finetune:
            ...
            self.test_predictions.append(self.forward_samples(batch, labels))
        else:
            # outputs = {}
            # outputs['predictions'], outputs['labels'] = self.generate_triples(batch, labels)
            predictions, labels = self.generate_triples(batch, labels)
    ...
    def on_validation_epoch_end(self) -> Any:
        if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
            scores, precision, recall, f1 = re_score([item for item in self.validation_predictions], [item for item in self.validation_labels], ['killed by', 'residence', 'location', 'headquarters location', 'employer'], "strict")
        elif self.hparams.dataset_name.split('/')[-1] == 'ade.py':
            scores, precision, recall, f1 = re_score([item for item in self.validation_predictions], [item for item in self.validation_labels], ['has effect'])
        elif self.hparams.dataset_name.split('/')[-1] == 'nyt_typed.py':
            scores, precision, recall, f1 = re_score([item for item in self.validation_predictions], [item for item in self.validation_labels], list(relations_nyt.values()), "strict")
        elif self.hparams.dataset_name.split('/')[-1] == 'docred_typed.py':
            ...
            scores, precision, recall, f1 = re_score([item for item in self.validation_predictions], [item for item in self.validation_labels], list(relations_docred.values()), "strict")
        else:
            scores, precision, recall, f1 = re_score([item for item in self.validation_predictions], [item for item in self.validation_labels], ['killed by', 'residence', 'location', 'headquarters location', 'employer'])
    ...
    def on_test_epoch_end(self, output: dict) -> Any:
        if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
            scores, precision, recall, f1 = re_score([item for item in self.validation_predictions], [item for item in self.validation_labels], ['killed by', 'residence', 'location', 'headquarters location', 'employer'], "strict")
        elif self.hparams.dataset_name.split('/')[-1] == 'ade.py':
            scores, precision, recall, f1 = re_score([item for item in self.validation_predictions], [item for item in self.validation_labels], ['has effect'])
